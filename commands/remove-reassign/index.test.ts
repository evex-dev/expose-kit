...
describe("Remove Reassign Command", () => {
	let program: Command;
	...
	it("Should keep runtime behavior after transformation", async () => {
		...
		await program.parseAsync([
			...
			"--output",
			sampleRemoveReassignAbsolutePath,
			"--unlimited",
		]);

		const originalResult = runScript(sampleMockFileAbsolutePath);
		const transformedResult = runScript(sampleRemoveReassignAbsolutePath);

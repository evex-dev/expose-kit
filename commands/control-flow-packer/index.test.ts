...
describe("Control Flow Packer Command", () => {
	let program: Command;
	...
	it("Should keep runtime behavior after transformation", async () => {
		...
		await program.parseAsync([
			...
			"--output",
			sampleControlFlowPackerAbsolutePath,
			"--unlimited",
		]);

		const originalResult = runScript(sampleMockFileAbsolutePath);
		const transformedResult = runScript(sampleControlFlowPackerAbsolutePath);

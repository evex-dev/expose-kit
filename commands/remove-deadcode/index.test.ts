...
describe("Remove Deadcode Command", () => {
	let program: Command;
	...
	it("Should keep runtime behavior after transformation", async () => {
		...
		await program.parseAsync([
			...
			"--output",
			sampleRemoveDeadcodeAbsolutePath,
			"--unlimited",
		]);

		const originalResult = runScript(sampleMockFileAbsolutePath);
		const transformedResult = runScript(sampleRemoveDeadcodeAbsolutePath);
